****************
* Jordan Machita
* Replication of File 1
* Importing and Saving Raw CCP Data
****************

cd "/Users/jmachita03/Desktop/Capstone/Stata Do Files & Data"
clear

* Import CRTSE data
import excel "2018_2019_CRTSE_SummerSecondary_PGMT_4thyrBMT_ItemsRemoved.xls", sheet("Sheet1") firstrow
rename *, lower
destring student, replace
egen crtse_total=rowmean(crtse_05-crtse_41)
drop if student==.
save "2018_2019_CRTSE_Data.dta", replace


* Import DAS data
import excel "2018_2019_DAS_SummerSecondary_PGMT_4thyrBMT_Recoded_Missing_ItemNamesRemoved.xls", sheet("Sheet1") firstrow clear
rename *, lower
drop if student==""
destring student, replace
...
save "2018_2019_DAS_Data.dta", replace

** Demographic Data
* Importing and saving demog data
import excel "2018_2019_Demography_Survey_SummerSecondary_PGMT.xls", sheet("Sheet1") firstrow clear
drop if _n==1
rename *,lower
save "2018_2019_Demography_Survey_Data1.dta", replace

* Importing and saving data from 2017-2018 for missing data
import excel "2017_2018_CCS1_DemographySurvey_NumID_NoConsentRemoved.xlsx", firstrow clear
drop if _n==1
rename *,lower
save "2017_2018_Demography_Survey_Data2.dta", replace

* Appending both datasets
append using "2018_2019_Demography_Survey_Data1.dta"
drop if student=="2176293" & assessed=="16sep2017"
drop if student==""
destring student, replace

* Saving final dataset
save "2018_2019_Demography_Survey_Data.dta", replace 


** FIT data
* Importing data
import excel "2018_2019_FIT_SummerSecondary_PGMT_4thyrBMT_Recoded_ItemNamesRemoved.xls", sheet("Sheet1") firstrow clear
rename *, lower
drop if student==""
destring student, replace
egen fit_total=rowmean(fit_01-fit_13)
save "2018_2019_FIT_Data.dta", replace 


** GRIT data
* Importing and saving GRIT data
import excel "2018_2019_GRIT_SummerSecondary_PGMT_4thyrBMT_ReverseCoded_OrigRemoved.xls", sheet("Sheet1") firstrow clear
rename *, lower
drop if student==""
destring student, replace
egen grit_total=rowmean(r_grit_02-r_grit_11)
save "2018_2019_GRIT_Data.dta", replace


** IMTS data
* Importing and saving IMTS data
import excel "2018_2019_IMTS_SummerSecondary_PGMT_4thyrBMT_Missing_ItemNamesRemoved.xls", sheet("Sheet1") firstrow clear
rename *, lower
egen imts_total=rowmean(imts_01-imts_05)
drop if student==""
destring student, replace
save "2018_2019_IMTS_Data.dta", replace

** NEO data
* Importing data
import excel "2018_2019_NEO_SummerSecondary_PGMT_ReverseCoded_OrigRemoved.xls", sheet("Sheet1")firstrow clear
drop if _n==1
drop if student==""
rename *, lower
destring student, replace
...
drop if _merge==2
drop _merge
sort recipientlastname
save "2018_2019_NEO1_Data.dta", replace
import excel "NEO 2017 09 27.xlsx", firstrow clear
drop if _n==1
rename *, lower
...
drop if r_neo_01==.
save "2018_2019_NEO2_Data.dta", replace
* Appending datasets
append using "2018_2019_NEO1_Data.dta"
...
save "2018_2019_NEO_Data.dta", replace
* Merging with missing data to update
use "2018_2019_NEO_Missing.dta"
rename *,lower
egen neo_n=rowmean(r_neo_01 neo_06 neo_11 r_neo_16 neo_21 neo_26 r_neo_31 neo_36 neo_41 r_neo_46 neo_51 neo_56)
egen neo_e=rowmean(neo_02 neo_07 r_neo_12 neo_17 neo_22 r_neo_27 neo_32 neo_37 r_neo_42 neo_47 neo_52 r_neo_57)
egen neo_o=rowmean(r_neo_03 r_neo_08 neo_13 r_neo_18 r_neo_23 neo_28 r_neo_33 r_neo_38 neo_43 r_neo_48 neo_53 neo_58)
...
save "2018_2019_NEO_Missing_Cleaned.dta", replace
* Merging two datasets
use "2018_2019_NEO_Data.dta"
merge 1:1 student using "2018_2019_NEO_Missing_Cleaned.dta", update
drop _merge
save "2018_2019_NEO_Data.dta", replace
...
** RSQ data
* Importing data
import excel "2018_2019_RSQ_SummerSecondary_PGMT_ReverseCoded_OrigRemoved.xls", sheet("Sheet1") firstrow clear
rename *, lower
save "2018_2019_RSQ1_Data.dta", replace
import excel "2018_2019_RSQ_Missing.xlsx", firstrow clear
rename *, lower
save "2018_2019_RSQ2_Data.dta", replace
* appending data
append using "2018_2019_RSQ1_Data.dta"
replace student="2603880" if name=="Gross,Hannah"
destring student, replace
drop if student==.
egen rsq_total=rowmean(r_rsq_01-rsq_30)
* saving dataset
save "2018_2019_RSQ_Data.dta", replace

** TMAS data
* Importing data
import excel "2018_2019_TMAS_SummerSecondary_PGMT_4thyrBMT_Missing_ReverseCoded_OrigRemoved_ItemNamesRemoved.xls", sheet("Sheet1") firstrow clear
rename *, lower
destring student, replace
drop if student==.
egen tmas_total=rowmean(tmas_01-r_tmas_20)
save "2018_2019_TMAS_Data.dta", replace


** TSE data
* Importing data
import excel "2018_2019_TSE_SummerSecondary_PGMT_4thyrBMT.xls", sheet("Sheet1") firstrow clear
rename *, lower
destring student, replace
drop if student==.
...
save "2018_2019_TSE_Data.dta", replace

** YTRT data
* Importing data
import excel "2018_2019_YTRT_SummerSecondary_PGMT_4thyrBMT_Missing_ItemNamesRemoved.xls", sheet("Sheet1") firstrow clear
rename *, lower
destring student, replace
drop if student==.
egen ytrt_total=rowmean(ytrt_01-ytrt_05)
save "2018_2019_YTRT_Data.dta", replace
keep student ytrt_total recipientlastname recipientfirstname 


* Merging all datasets
merge 1:1 student using "2018_2019_CRTSE_Data.dta", keepusing (crtse_total)
drop _merge
merge 1:1 student using "2018_2019_DAS_Data.dta", keepusing(das_depression das_anxiety das_stress)
drop _merge
merge 1:1 student using "2018_2019_Demography_Survey_Data.dta", keepusing (ccs_gpa-hsach)
drop if _merge==2
drop _merge
merge 1:1 student using "2018_2019_FIT_Data.dta", keepusing(fit_total)
drop _merge
merge 1:1 student using "2018_2019_GRIT_Data.dta", keepusing (grit_total)
drop _merge
merge 1:1 student using "2018_2019_IMTS_Data.dta", keepusing (imts_total)
drop _merge 
merge 1:1 student using "2018_2019_NEO_Data.dta", keepusing (neo_n neo_e neo_o neo_a neo_c)
drop if _merge==2
drop _merge
merge 1:1 recipientlastname recipientfirstname using "2018_2019_RSQ_Data.dta", keepusing(rsq_total)
drop if _merge==2
drop _merge
merge 1:1 recipientlastname recipientfirstname using "2018_2019_TMAS_Data.dta", keepusing(tmas_total)
drop _merge
merge 1:1 recipientlastname recipientfirstname using "2018_2019_TSE_Data.dta", keepusing(tses_se tses_is tses_cm tses_total)
drop _merge
merge 1:1 recipientlastname recipientfirstname using "2018_2019_YTRT_Data.dta", keepusing (ytrt_total)
drop _merge
...
merge 1:1 email using "SimTeacher Randomization Fall 2018 Spring 2019.dta"
drop _merge
save "SimTeacher Randomization CPP Fall 2018 Spring 2019.dta", replace
